...
quick_sort(data_t A[], int n) {
    ...
    // Recursion depth guaranteed logn
    while (n > 1) {
        ...
        int left_size = first_eq;
        int right_size = n - first_gt;

        // recurse on left, itr on right
        if (left_size < right_size) {
            quick_sort(A, left_size);
            A = A + first_gt;
            n = right_size;   
        // recurse on right, itr on left
        } else {
            quick_sort(A+first_gt, right_size);
            n = left_size;
        }
    }
}
